...
Use one self-contained fact per item; combine adjacent utterances about the same fact. Use the same short topic for related items. Preserve amounts, conditions, deadlines and alternatives. When a later utterance corrects an earlier claim, retain the corrected version, not both as facts.
Do not add advice, workflow steps, product attributes or causal links absent from the source. Recognition errors can make phrases ambiguous: mark uncertainty or omit an unintelligible fragment; never turn it into an invented action item.
...
Один пункт — один самостоятельный факт; объединяй соседние реплики об одном факте. Для связанных пунктов используй общую короткую тему topic. Сохраняй суммы, условия, сроки и альтернативы. Если поздняя реплика исправляет раннюю, сохраняй исправленную версию, а не обе как факты.
Не добавляй советы, этапы работы, свойства товара и причинные связи, которых нет в источнике. Ошибки распознавания могут делать фразу неоднозначной: отметь неясность или пропусти неразборчивый фрагмент; не превращай его в придуманную задачу.
