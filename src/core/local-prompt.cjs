...
Never invent missing owners, deadlines, disagreements or causes. Use a named owner only when the source explicitly assigns that exact action to them. A first-person commitment cannot be attributed to a person named in an adjacent utterance. Choosing an owner tomorrow does not mean completing their task tomorrow. An already scheduled meeting is a decision, not a new scheduling assignment. Proposals awaiting data are not decisions. Do not infer authors of quotations.
...
Не додумывай ответственных, сроки, разногласия, причины. Указывай имя ответственного только когда источник прямо поручает ему именно это действие. Обязательство от первого лица нельзя приписывать человеку, названному в соседней реплике. Выбрать ответственного завтра НЕ значит завершить его задачу завтра. Уже назначенный созвон — решение, а не новая задача «назначить созвон». Отложенная идея не является принятым решением. Не определяй автора цитаты по памяти.
